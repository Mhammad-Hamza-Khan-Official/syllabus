...
export default function IndividualQuizBottomNavigation() {
  ...
  // For move Question to next .
  useEffect(() => {
    const handleKeyDown = (e) => {
      if (
        e.key === "Enter" &&
        !(
          state.currentQuestion + 1 >= totalNumberOfQuestions || isNotableToNext
        )
      ) {
        nextQuestionsHandle();
      }
  ...
  }, [nextQuestionsHandle, state, totalNumberOfQuestions, isNotableToNext]);
